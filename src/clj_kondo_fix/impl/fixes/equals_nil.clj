(ns clj-kondo-fix.impl.fixes.equals-nil
...
(defn fix-equals-nil-in-file [file-path lines findings log]
            ...
            (let [open-col  col-idx
              ...
              (if (nil? close-pos)
                ...
                (let [[close-line close-col] close-pos]
                  ;; Only handle single-line forms
                  (if (not= close-line line-idx)
                    [current-lines nil]
                    (let [line  (nth current-lines line-idx)
                          inner (subs line (inc open-col) close-col)]
                      ;; inner = "= nil ARG" or "= ARG nil"
                      (cond
                        ;; Case 1: (= nil ARG) → (nil? ARG)
                        (.startsWith inner "= nil ")
                        (let [arg      (subs line (+ open-col 7) close-col)
                        ...
                        ;; Case 2: (= ARG nil) → (nil? ARG)
                        (.endsWith inner " nil")
                        (let [arg      (subs line (+ open-col 3) (- close-col 4))
                              new-line (str (subs line 0 open-col)
                                            "(nil? " arg ")"
                                            (subs line (inc close-col)))]
                          (swap! log conj (str "  " fu ":" (:line f)
