(ns clj-kondo-fix.impl.fixes.unused-excluded-var
...
(defn- extract-var-name [msg]
  (second (re-find #"^Unused excluded var: (.+)$" msg)))

(defn fix-unused-excluded-var-in-file [file-path lines findings log]
                             ...
                             (if (= new-line line)
                               ...
                               (let [new-lines
                                 ...
                                 (swap! log conj (str "  " fu ":" (:line f)
                                                      "  remove unused excluded var: " var-name))
                                 [new-lines true]))))))
