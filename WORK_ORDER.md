...
5. **Remove the rule in the same commit.** The commit that ships the fix must also
   delete the rule's entry from this file. Do not batch removals.
...
### 1. `unused-alias`
- **Trigger:** an `:as x` alias in a `:require` entry is never used in the file.
- **Fix:** remove the `:as x` pair. If the entry has no remaining `:refer`,
  fall back to dropping the bare entry via the existing require cleanup.
- **Reuse:** `require_entry.clj` helpers.
- **Effort:** XS.
